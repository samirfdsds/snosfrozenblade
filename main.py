...
async def get_user_data(user_id: int):
    ...
    data = json.loads(raw) if raw else {"strikes": 0, "refs": 0, "sub_expires": 0, "last_refill": "", "is_admin": False}
    if data.get("sub_expires", 0) > time.time():
        ...
        if data.get("last_refill") != today:
            ...
            try: await bot.send_message(user_id, "🎁 VIP БОНУС: +5 ударов!")
            except: pass
    return data
...
async def admin_give(m: Message, cmd: CommandObject):
    u_data = await get_user_data(m.from_user.id)
    if m.from_user.id != MY_ID and not u_data.get("is_admin"): return
    try:
        uid, count = cmd.args.split()
        target = await get_user_data(int(uid))
        target["strikes"] += int(count)
        await save_user_data(int(uid), target)
        await m.answer(f"✅ Выдано {count} ударов юзеру `{uid}`")
    except: await m.answer("Ошибка. Формат: `/monifoldgive ID колво`")
...
async def cmd_start(m: Message):
    ...
    kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="☢️ НАНЕСТИ УДАР"), KeyboardButton(text="💎 МАГАЗИН")], [KeyboardButton(text="👤 ПРОФИЛЬ")]], resize_keyboard=True)
    await m.answer("⚔️ **GOD ENGINE ONLINE**", reply_markup=kb)

@dp.callback_query(F.data == "check_sub")
async def check_sub_btn(c: CallbackQuery):
    if await check_subscription(c.from_user.id):
        await c.answer("✅ Доступ разрешен!"); await c.message.delete(); await cmd_start(c.message)
...
async def process_buy(c: CallbackQuery):
    ...
    url = await create_lava_invoice(price, order_id)
    if url: await c.message.answer(f"🛒 **Счет на {price}₽ создан!**", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💳 ОПЛАТИТЬ", url=url)]]))
    await c.answer()
...
async def attack_init(m: Message, state: FSMContext):
    ...
    await m.answer("📡 Пришлите `@username` цели:"); await state.set_state(AttackStates.waiting_for_target)
...
async def target_capture(m: Message, state: FSMContext):
    ...
    target_id = "Unknown"
    async with aiohttp.ClientSession() as s:
        async with s.get(f"https://t.me/{user}") as r:
            t = await r.text(); match = re.search(r'tg://resolve\?domain=.*?id=(\d+)', t)
            if match: target_id = match.group(1)
    await m.answer(f"🎯 **ЦЕЛЬ:** @{user}\n🆔 **ID:** `{target_id}`", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💥 ЗАПУСК", callback_data=f"fire_{user}_{target_id}")]]))
    await state.clear()
...
async def fire_strike(c: CallbackQuery):
    ...
    await c.message.edit_text("🚀 **ВЫПОЛНЯЕТСЯ...**")
    msg = MIMEMultipart(); msg['From'] = EMAIL_SENDER; msg['To'] = "abuse@telegram.org"; msg['Subject'] = f"REPORT-UID-{uid}"
    ...
    try:
        ...
        if c.from_user.id != MY_ID:
            data["strikes"] -= 1; await save_user_data(c.from_user.id, data)
        await c.message.edit_text(f"✅ **ОТПРАВЛЕНО!**")
        try: await bot.send_message(CHANNEL_ID, f"🔥 **STRIKE LOG**\nTarget: @{user}\nID: `{uid}`")
        except: pass
    except: await c.message.edit_text("❌ Ошибка SMTP.")
...
async def run_web():
    ...
    webapp.router.add_get('/', lambda r: aiohttp.web.Response(text="ONLINE"))
    runner = aiohttp.web.AppRunner(webapp)
    await runner.setup()
    await aiohttp.web.TCPSite(runner, '0.0.0.0', int(os.environ.get("PORT", 8080))).start()

async def main():
    asyncio.create_task(run_web()); await dp.start_polling(bot)
